...
function writeFixture(relPath, content = '') {
  const root = path.resolve(process.cwd(), 'packages', 'eslint-plugin-local-import-ext', 'test', 'fixtures');
...
test('discovered extension is preferred when existing', () => {
  ...
  ruleTester.run('require-local-extension', rule, {
    ...
    invalid: [
      {
        filename: path.join('packages', 'eslint-plugin-local-import-ext', 'test', 'fixtures', 'prefer', 'tstarget.js'),
...
test('directory index resolution', () => {
  ...
  ruleTester.run('require-local-extension', rule, {
    ...
    invalid: [
      {
        filename: path.join('packages', 'eslint-plugin-local-import-ext', 'test', 'fixtures', 'widget', 'caller.js'),
